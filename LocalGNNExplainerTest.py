...
Original file is located at
    https://colab.research.google.com/drive/1N92IxpAQkct6XeyRRRNIuKEdBymKVvXM
"""
...
# Load & Preprocess
# ======================
df = pd.read_csv("test2 (4).csv")
truth = pd.read_csv("testTruth (2).csv")
...
MODEL_PATH = "bettergin_weights.pt"
torch.save(model.state_dict(), MODEL_PATH)
print("Model weights saved to:", MODEL_PATH)

# 2. Reload model (optional but recommended for sanity)
model = BetterGIN(
...
model.eval()
print("Model weights loaded and model set to eval()")


# 3. Create GNNExplainer
explainer = Explainer(
    model=model,
    ...
    node_mask_type="attributes",   # feature (PCA) importance
    edge_mask_type="object",        # edge importance
    model_config=dict(
        mode="regression",
        task_level="node",
        return_type="raw"           # model outputs raw values
    )
)

# 4. Choose a test node to explain
node_idx = int(test_mask[0])
print("Explaining node index:", node_idx)
...
# 6. Extract explanation masks
edge_mask = explanation.edge_mask          # [num_edges]
node_feat_mask = explanation.node_mask     # [num_features]

print("Edge mask shape:", edge_mask.shape)
print("Node feature mask shape:", node_feat_mask.shape)

# 7. Top-K most important edges
K = 10
top_edges = edge_mask.argsort(descending=True)[:K]

print(f"\n Top {K} most important edges (global edge indices):")
for i, eidx in enumerate(top_edges):
    print(f"{i+1:02d}. Edge {eidx.item()} — importance {edge_mask[eidx].item():.4f}")

# ----------------------
# 8. Feature importance (PCA components)
# ----------------------
print("\n Feature importance (PCA space):")
for i, score in enumerate(node_feat_mask):
    print(f"PCA component {i:02d}: importance = {score.item():.4f}")

# ----------------------
# 9. Extract k-hop subgraph used for explanation
# ----------------------
...
print("\n Explanation subgraph stats:")
print("Subgraph nodes:", subset.size(0))
print("Subgraph edges:", edge_index_sub.size(1))

# ----------------------
# 10. Sanity check prediction
# ----------------------
with torch.no_grad():
    pred = model(data.x, data.edge_index)[node_idx].item()

print(f"\n Model prediction for node {node_idx}: {pred:.4f}")